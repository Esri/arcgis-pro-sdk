...
{
  /// <summary>
  /// Illustrates how to check whether a value has changed in a given field of a Row or Feature.
  /// </summary>
  /// 
  /// <remarks>
  /// <para>
        ...
        editOperation.Callback(context => 
        {
          ...
          using (RowCursor rowCursor = enterpriseTable.Search(openCutFilter, false))
          {
            ...
            while (rowCursor.MoveNext())
            {
              ...
              {
                // Will be false.
                bool beforeChange = row.HasValueChanged(assetNameIndex);
                ...
                // Will be true.
                bool afterChange = row.HasValueChanged(assetNameIndex); 

                row.Store(); //After all the changes are done, persist it

                // Will be false.
                bool afterStore = row.HasValueChanged(assetNameIndex);
