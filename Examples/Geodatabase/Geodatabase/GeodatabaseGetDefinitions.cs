...
namespace SDKExamples
{
  ...
  /// </para>
  /// </remarks>
  public class GeodatabaseGetDefinitions
  {
    /// <summary>
    /// In order to illustrate that Geodatabase calls have to be made on the MCT
    /// </summary>
    /// <returns></returns>
    public async Task GeodatabaseGetDefinitionsAsync()
    {
    ...
    public void MainMethodCode()
    {
      ...
      {
        IReadOnlyList<TableDefinition> definitions = fileGeodatabase.GetDefinitions<TableDefinition>();

        foreach (var definition in definitions)
        {
          // Do something with the tableDefinition.
 
          if (definition.HasGlobalID())
          {
      ...
      using (Geodatabase geodatabase = new Geodatabase((connectionProperties)))
      {
        ...
        list.AddRange(enterpriseDefinitions);
        list.AddRange(featureDatasetDefinitions);
        list.AddRange(relationshipClassDefinitions);
        list.AddRange(attributedRelationshipClassDefinitions);
        // Do something with the consolidated list.

        try
        {
          // Cannot access Unknown Datasets.
          geodatabase.GetDefinitions<UnknownDefinition>();
        }
